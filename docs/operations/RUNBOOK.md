...
## 5. Authentication และ Role Boundary

Admin ใช้ email/password จริงกับ JWT ที่ frontend เก็บใน `sessionStorage` และส่งผ่าน `Authorization: Bearer <token>`. API โหลด role, agency และ active state ล่าสุดจาก `admin_users` ทุก request จึงไม่เชื่อ scope ที่ browser ส่งเอง และการ deactivate มีผลใน request ถัดไปทันที:
...
`EventSource` ส่ง Authorization header ไม่ได้ จึงต้องเรียก `POST /api/auth/sse-ticket` ก่อน แล้วเปิด `GET /api/events?ticket=<one-time-ticket>`. Ticket มีอายุสั้น ใช้ได้ครั้งเดียว และห้ามนำ access JWT ไปใส่ URL

SSE ticket store ปัจจุบันอยู่ใน memory ของ API instance เดียว หากเพิ่ม replica ต้องย้าย ticket/rate-limit state ไป shared store เช่น Redis ก่อน ไม่เช่นนั้น ticket ที่ออกจาก instance หนึ่งอาจใช้กับอีก instance ไม่ได้

ก่อน rollout release นี้ต้อง apply `021_admin_user_auth.sql`, ตั้ง `JWT_SECRET` และ `ADMIN_BOOTSTRAP_*`, rebuild/recreate API แล้วทดสอบ login บัญชี bootstrap จากนั้นจึง deploy frontend ห้าม deploy frontend/API แยก contract เพราะ mock scope header ถูกตัดออกแล้ว
