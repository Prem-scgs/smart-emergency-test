/**
 * ล็อก settings route shell, permission รายส่วน และพฤติกรรม preference/sound
 * เพื่อกัน personal settings กับ system settings ถูกเปิดให้ role ผิดกลุ่มหลัง refactor
 */
...
test('settings page gates browser preferences and system settings with separate permissions', () => {
  assert.match(page, /useAuth/)
  assert.match(page, /canViewAllAgencies/)
  assert.match(page, /hasPermission\("settings\.personal\.edit"\)/)
  assert.match(page, /hasPermission\("settings\.organization\.edit"\)/)
  assert.match(page, /hasPermission\("settings\.share-channels\.edit"\)/)
  assert.match(page, /hasPermission\("settings\.health\.view"\)/)
...
test('settings page lets super admin edit organization settings through the save flow', () => {
  ...
  assert.match(page, /<TabsList className=\{hasPrivilegedSettings \? "grid w-full grid-cols-3"/)
