/**
 * ???? contacts UI ?????? admin i18n ??? localized reference labels ??? raw codes.
 */
import { readFileSync } from 'node:fs'
import { test } from 'node:test'
import assert from 'node:assert/strict'

const contactsRoute = readFileSync('app/admin/(dashboard)/contacts/page.tsx', 'utf8')
const contactsPage = readFileSync('widgets/admin-contacts/ui/contacts-page.tsx', 'utf8')
const i18n = [
...
test('contacts page uses admin i18n and localized master data labels', () => {
  assert.match(contactsRoute, /@\/widgets\/admin-contacts/)
