/**
 * ???? call logs route shell, scoped API headers ??? export/print behavior ??????? widget.
 */
import { readFileSync } from 'node:fs'
import { test } from 'node:test'
import assert from 'node:assert/strict'

const route = readFileSync('app/admin/(dashboard)/call-logs/page.tsx', 'utf8')
const page = readFileSync('widgets/admin-call-logs/ui/call-logs-page.tsx', 'utf8')
const exportHelpers = readFileSync('widgets/admin-call-logs/lib/export.ts', 'utf8')
const i18n = [
...
test('call logs route is a thin widget shell', () => {
  assert.match(route, /@\/widgets\/admin-call-logs/)
  assert.match(route, /<CallLogsPage \/>/)
  assert.doesNotMatch(route, /fetch\(/)
  assert.doesNotMatch(route, /exportCsv/)
  assert.doesNotMatch(route, /printCallLogs/)
})
...
test('call logs PDF and print use safe offscreen pages instead of opening blank windows', () => {
  ...
  assert.match(exportHelpers, /buildPrintableHtml/)
  assert.doesNotMatch(page, /window\.open/)
  assert.doesNotMatch(page, /about:blank/)
})

test('call logs print snapshot keeps light document colors while admin is in dark theme', () => {
  assert.match(page, /html\.dark \.print-only-call-logs/)
