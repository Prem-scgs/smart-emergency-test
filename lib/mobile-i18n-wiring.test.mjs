...
test('mobile language control uses a readable dropdown instead of squeezing text into an icon button', async () => {
  const app = await read('widgets/mobile-emergency/ui/mobile-app.tsx')

  assert.match(app, /size="sm"/)
  assert.match(app, /import \{ Languages, Moon, Sun \} from "lucide-react"/)
  assert.match(app, /<Languages className="h-4 w-4 text-muted-foreground" \/>/)
  assert.match(app, /SelectItem value="th"/)
  assert.match(app, /SelectItem value="en">English/)
})
