...
test('reports page enables csv export from current report summary', () => {
  ...
  assert.match(page, /toast\.success\("ส่งออกรายงาน CSV แล้ว"\)/)
...
test('reports page has polished loading, empty, and retry states', () => {
  assert.match(page, /กำลังโหลดรายงาน/)
  assert.match(page, /ไม่มีข้อมูลสำหรับส่งออก/)
