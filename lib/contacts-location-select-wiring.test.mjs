/**
 * ???? province/district selector ??? contacts ????????????????????? master data.
 */
...
test('contacts location selects render Thai labels instead of raw province or district codes', () => {
  ...
  assert.match(page, /getLocationDisplayName\(province, preferThai\)/)
  assert.match(page, /getLocationDisplayName\(district, preferThai\)/)
